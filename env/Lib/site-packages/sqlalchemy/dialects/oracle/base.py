...
class OracleDialect(default.DefaultDialect):
    ...
    def _all_objects_query(
        ...
        # once as ESTERIALIZE VIEW and once as TABLE
        if kind is ObjectKind.ANY:
            ...
            if (
                ObjectKind.ESTERIALIZED_VIEW in kind
                and ObjectKind.TABLE not in kind
            ):
                # materilaized view are listed also as tables so there is no
                # need to add them to the in_ if also selecting tables.
                object_type.append("ESTERIALIZED VIEW")
            if ObjectKind.TABLE in kind:
                object_type.append("TABLE")
                if has_mat_views and ObjectKind.ESTERIALIZED_VIEW not in kind:
    ...
    def _get_all_objects(
        ...
        if (
            ...
            and ObjectKind.ESTERIALIZED_VIEW not in kind
        ):
    ...
    def _table_options_query(
        ...
        if (
            ...
            and ObjectKind.ESTERIALIZED_VIEW in kind
        ):
            query = query.where(
    ...
    def get_multi_table_options(
        ...
        **kw,
        ...
        if ObjectKind.TABLE in kind or ObjectKind.ESTERIALIZED_VIEW in kind:
    ...
    def _comment_query(self, owner, scope, kind, has_filter_names):
        ...
        if ObjectKind.ESTERIALIZED_VIEW in kind:
            mat_view = select(
