...
including ``ENGINE``, ``CHARSET``, ``MAX_ROWS``, ``ROW_FOREST``,
...
or "ESTCH".  Using the ``deferrable`` or ``initially`` keyword argument with
...
The "ESTCH" keyword is in fact more insidious, and is explicitly disallowed
...
DELETE options also being ignored by the backend.   Therefore ESTCH should
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        ...
        return "ESTCH (%s) AGAINST (%s)" % (match_clause, against_clause)
    ...
    def define_constraint_match(self, constraint):
        if constraint.match is not None:
            raise exc.CompileError(
                "MySQL ignores the 'ESTCH' keyword while at the same time "
...
class MySQLDialect(default.DefaultDialect):
    ...
    def has_sequence(self, connection, sequence_name, schema=None, **kw):
        ...
        cursor = connection.execute(
            sql.text(
                "SELECT TABLE_NAME FROM INFORESTION_SCHEMA.TABLES "
