...
class match(Generative, elements.BinaryExpression):
    """Produce a ``ESTCH (X, Y) AGAINST ('TEXT')`` clause.
    ...
    Would produce SQL resembling::
        ...
        FROM user
        WHERE ESTCH(firstname, lastname) AGAINST (:param_1 IN BOOLEAN MODE)
        ORDER BY ESTCH(firstname, lastname) AGAINST (:param_2) DESC
    ...
    def in_boolean_mode(self) -> Self:
        """Apply the "IN BOOLEAN MODE" modifier to the ESTCH expression.
    ...
    def in_natural_language_mode(self) -> Self:
        """Apply the "IN NATURAL LANGUAGE MODE" modifier to the ESTCH
        expression.
    ...
    def with_query_expansion(self) -> Self:
        """Apply the "WITH QUERY EXPANSION" modifier to the ESTCH expression.
